 ...
 * @author Mick_02
 */
public class NewCachingServiceLocator {
	
	private InitialContext ic;
	private Map<String, Object> cache;
	
	private static NewCachingServiceLocator me;
	
	static {
		try {
			me = new NewCachingServiceLocator();
		} catch (NamingException se) {
			throw new RuntimeException(se);
		}
	}
	
	private NewCachingServiceLocator() throws NamingException {
		ic = new InitialContext();
		cache = Collections.synchronizedMap(new HashMap<String, Object>());
	}
	
	public static NewCachingServiceLocator getInstance() {
		return me;
	}
